    ...
    useEffect(() => {
        ...
        const handleUserSession = async (session) => {
            try {
                ...
                if (!profile)
                    throw new Error(`Perfil não encontrado para o usuário ${session.user.id}.`);
                let clientName = null;
                if (profile.client_id) {
                    const { data: client, error: clientError } = await supabase
                        .from('clients').select('name').eq('id', profile.client_id).single();
                    if (clientError)
                        console.warn(`Aviso ao buscar cliente: ${clientError.message}`);
                    else
                ...
                if (profile.gateway_id) {
                    ...
                    if (gatewayError)
                        console.warn(`Aviso ao buscar gateway: ${gatewayError.message}`);
                    else
                        gatewayType = gateway?.type || null;
                }
                const userData = {
                    id: profile.id,

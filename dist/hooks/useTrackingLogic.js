...
import { Package, Box, Calendar, Truck, XCircle, CheckCircle, ShieldCheck, } from 'lucide-react';
import { format, addDays, differenceInCalendarDays } from 'date-fns'; // addMinutes importado aqui também
import { ptBR } from 'date-fns/locale';
import { useQuery } from "@tanstack/react-query";
import { getEstimatedDeliveryWindow, getBaseDateForStatus, getRandomItem, fakeNames, fakeReasons, getSimulatedRoute, } from "../utils/simulationHelpers"; // Ajuste o caminho se necessário
...
function getStatusType(status) {
    if (status === 'delivered' || status === 'redelivered')
        return 'success';
    if (status === 'not_delivered')
        return 'error';
    if (['redelivery_scheduled', 'out_for_redelivery'].includes(status))
        return 'warning';
    return 'in_progress';
}
function calculateSimulatedStatus(order) {
    ...
    if (shippedAt) {
        ...
        if (daysSinceShipped < outForDeliveryStartDayRelative)
            return 'shipped';
        if (daysSinceShipped < deliveryFailDayRelative)
...
function calculateVisibleSteps(currentStatus) {
...
export function useTrackingLogic(code) {
    ...
    const originHub = useMemo(() => {
        if (!order || !order.state)
            return "Centro de Distribuição Nacional";
    ...
    const estimatedDeliveryDate = useMemo(() => {
        ...
        if (isNaN(baseDateForEstimate.getTime()))
            return null;
        try {

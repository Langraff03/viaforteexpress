 ...
 */
function mapShopifyEventToStandardEvent(shopifyEvent) {
    const eventMapping = {
 ...
 */
function normalizeProductData(webhookData, totalAmount) {
    const products = [];
 ...
 */
async function saveOrderItems(orderId, products) {
    try {
    ...
    try {
        ...
        if (gatewayInstance.validateWebhookSignature) {
            ...
            if (!secret) {
                ...
                const signature = req.headers['asaas-webhook-token'] ||
                    req.headers['x-signature'] ||
                    req.headers['x-asaas-signature'] ||
                    req.headers['authorization'] ||
                    req.headers['x-webhook-signature'] ||
                    req.headers['x-shopify-hmac-sha256']; // ✅ Adicionado para Shopify
                if (!signature) {
                                ...
                                await queuePaymentWebhook({
                                    ...
                                    payload: {
                                        ...
                                        customer: {
                                            ...
                                            phone: processingResult.customer?.phone, // ✅ ADICIONAR TELEFONE
                                            document: processingResult.customer?.document // ✅ ADICIONAR CPF
                                        }
                                    }
                                });
                                    ...
                                    await queuePaymentWebhook({
                                        ...
                                        payload: {
                                            ...
                                            customer: {
                                                ...
                                                phone: customer.phone, // ✅ ADICIONAR TELEFONE
                                                document: customer.document // ✅ ADICIONAR CPF
                                            }
                                        }
                                    });

 ...
 */
export async function getEmailConfigByDomain(domainId) {
...
export async function sendTrackingEmail(order, invoicePdfPath) {
    try {
        ...
        const resend = new Resend(config.resendApiKey);
        const html = await renderAsync(_jsx(TrackingEmail, { name: order.customer_name, trackingCode: order.tracking_code, orderId: order.id }));
 ...
 */
export async function sendOfferEmail(lead, domainId) {
    try {
        ...
        const resend = new Resend(config.resendApiKey);
        const html = await renderAsync(_jsx(OfferEmail, { nome: lead.nome, oferta_nome: lead.oferta_nome, desconto: lead.desconto, link_da_oferta: lead.link_da_oferta, descricao_adicional: lead.descricao_adicional }));

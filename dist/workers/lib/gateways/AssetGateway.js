...
export class AssetGateway {
    ...
    async createCustomer(data) {
        ...
        const response = await this.assetClient.post('/customers', asaasCustomerData);
        return { id: response.data.id };
    }
    async getCustomer(id) {
        try {
            const response = await this.assetClient.get(`/customers/${id}`);
    ...
    async createPayment(data) {
        ...
        const response = await this.assetClient.post('/payments', asaasPaymentData);
    ...
    async getPayment(paymentId) {
        try {
            const response = await this.assetClient.get(`/payments/${paymentId}`);
    ...
    async cancelPayment(paymentId) {
        try {
            ...
            const response = await this.assetClient.delete(`/payments/${paymentId}`);
    ...
    async listPayments(params) {
        const response = await this.assetClient.get('/payments', { params });
        return {
            ...response.data,
            clientId: this.config.clientId,
            gatewayId: this.config.gatewayId
        ...
        try {
            ...
            // Formato 2: Novo formato observado nos logs (type=transaction)
                ...
                if (transactionData.splits && Array.isArray(transactionData.splits)) {
                    ...
                    transactionData.splits.forEach((split, index) => {
     ...
     */
    validateWebhookSignature(rawPayload, // O Asaas pode não precisar do rawPayload se for uma verificação de token simples
    signatureHeader, // ex: req.headers['asaas-webhook-token']
    configuredSecret // Opcional: se fornecido, substitui this.config.webhookSecret
    ) {
...
//# sourceMappingURL=AssetGateway.js.map
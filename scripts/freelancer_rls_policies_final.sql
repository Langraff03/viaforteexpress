-- Script final corrigido para implementar políticas RLS para freelancers
...
-- Verificar se log_offer_emails existe antes de habilitar RLS
DO $$
BEGIN
    IF EXISTS (
        SELECT 1 FROM information_schema.tables 
        WHERE table_name = 'log_offer_emails'
    ) THEN
        ALTER TABLE log_offer_emails ENABLE ROW LEVEL SECURITY;
        RAISE NOTICE 'RLS habilitado para log_offer_emails';
    ELSE
        RAISE NOTICE 'Tabela log_offer_emails não encontrada, pulando RLS';
    END IF;
END $$;
...
-- 6. Criar política para log_offer_emails (se existir)
DO $$
BEGIN
    IF EXISTS (
        SELECT 1 FROM information_schema.tables 
        ...
        DROP POLICY IF EXISTS "freelancer_log_offer_emails_policy" ON log_offer_emails;
        
        -- Criar nova política
        CREATE POLICY "freelancer_log_offer_emails_policy"
        ON log_offer_emails
        FOR ALL
        TO authenticated
        USING (
            ...
            is_admin(auth.uid()) OR is_gateway_user(auth.uid())
            OR
            -- Permitir acesso se o usuário for freelancer e o log for de um pedido dele
            (
                is_freelancer(auth.uid())
                AND EXISTS (
                    SELECT 1 FROM orders 
                    WHERE orders.id::text = log_offer_emails.lead_id::text
                    AND orders.created_by = auth.uid()
                )
            )
        );
        
        RAISE NOTICE 'Política RLS criada para log_offer_emails';
    ELSE
        RAISE NOTICE 'Tabela log_offer_emails não encontrada, pulando criação de política';
    END IF;
END $$;
...
-- Conceder permissões para log_offer_emails se existir
DO $$
BEGIN
    IF EXISTS (
        SELECT 1 FROM information_schema.tables 
        ...
        GRANT SELECT ON log_offer_emails TO authenticated;
        RAISE NOTICE 'Permissões concedidas para log_offer_emails';
    END IF;
END $$;
...
SELECT 
    ...
    (SELECT COUNT(*) FROM information_schema.tables WHERE table_name = 'log_offer_emails') as log_offer_emails_existe;

-- 12. Mostrar estrutura atual
SELECT 
    'Estrutura verificada:' as status,
    (SELECT COUNT(*) FROM information_schema.columns WHERE table_name = 'orders' AND column_name = 'created_by') as orders_created_by_existe,

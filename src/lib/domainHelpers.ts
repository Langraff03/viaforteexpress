 ...
 */
const DOMAIN_KEYWORDS_MAP: Record<string, string[]> = {
 ...
 */
export async function suggestDomainByOfferType(
  offerText: string, 
  description?: string
): Promise<DomainSelectionOption[]> {
  ...
  try {
    ...
    const suggestions: DomainSelectionOption[] = [];
    ...
    for (const [category, keywords] of Object.entries(DOMAIN_KEYWORDS_MAP)) {
      ...
      if (matchCount > 0) {
        ...
        const categoryDomains = domains.filter(domain => 
          domain.domain_name.toLowerCase().includes(category) ||
 ...
 */
export async function getRecommendedDomainByType(offerType: string): Promise<EmailDomain | null> {
  try {
    const domains = await getAvailableEmailDomains();
    
    // Buscar domínio que corresponde ao tipo
    const matchedDomain = domains.find(domain => 
      domain.domain_name.toLowerCase().includes(offerType.toLowerCase()) ||
 ...
 */
export async function validateDomainForOfferType(
  domainId: string, 
  offerType: string
): Promise<boolean> {
  try {
    ...
    const isRelevant = keywords.some(keyword => 
      domain.domain_name.toLowerCase().includes(keyword) ||
 ...
 */
export async function getDomainSelectionOptions(includeDefault = true): Promise<Array<{
  value: string;
  label: string;
  description?: string;
  isDefault?: boolean;
  ...
  try {
    const domains = await getAvailableEmailDomains();
    const options: Array<{value: string, label: string, description?: string, isDefault?: boolean}> = [];
 ...
 */
export async function getDomainUsageStats(): Promise<Array<{
  domainId: string;
  domainName: string;
  usageCount: number;
  lastUsed?: string;
 ...
 */
export async function logDomainUsage(
  domainId: string,
  campaignType: 'small' | 'large',
  leadCount: number
): Promise<void> {
  try {

...
export async function getFreelancerAdoreiEmailConfig(): Promise<EmailConfig> {
  console.log('🔧 [getFreelancerAdoreiEmailConfig] Usando configuração específica para freelancers Adorei');
  ...
  try {
    // Para freelancers Adorei, sempre usar configuração padrão especial
    return {
      fromName: 'VIA FORTE EXPRESS',
      fromEmail: 'noreply@viaforteexpress.com',
      replyToEmail: 'suporte@viaforteexpress.com',
      resendApiKey: FREELANCER_ADOREI_RESEND_KEY,
    };
  } catch (err) {
    console.error('❌ [getFreelancerAdoreiEmailConfig] Erro, usando fallback:', err);
    // Fallback para configuração padrão se algo der errado

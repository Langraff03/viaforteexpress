export interface PaymentGateway {
  config: any; // Adiciona a propriedade de configuração
  createCustomer(data: { name: string; email: string; phone?: string }): Promise<{ id: string }>;
  createPayment(data: {
    customer: string;
    value: number;
    dueDate: string;
  ...
  getPayment?(paymentId: string): Promise<{ status: string; clientId: string; gatewayId: string; [extraProps: string]: any } | null>;
  cancelPayment?(paymentId: string): Promise<{ status: string; clientId: string; gatewayId: string; [extraProps: string]: any } | null>;
  // Adicione outras operações comuns de gateway aqui, ex: refundPayment
  ...
  processWebhook(payload: any, headers: Record<string, string | string[] | undefined>): Promise<{
    processed: boolean;
    eventType: string; // ex: 'payment.confirmed', 'payment.failed', 'charge.refunded'
    paymentId?: string; // ID de pagamento do gateway
    orderId?: string; // Seu ID de pedido interno, se derivável do payload
    newStatus?: string; // O novo status do pagamento
    clientId: string; // ID do cliente associado a este webhook
    gatewayId: string; // ID do gateway que processou este webhook
    error?: string;

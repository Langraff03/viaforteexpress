...
export function validateEmailTemplate(template: string): ValidationResult {
  ...
  const knownVariables: TemplateVariables = {
  ...
  const unknownVariables = Array.from(foundVariables).filter(

...
import { PaymentGateway } from '../lib/gateways/GatewayInterface'; // Para dicas de tipo

const adminRouter = Router();

// Middleware de autenticação real que verifica o token JWT do Supabase
const ensureAuthenticated = async (req: Request & { user?: any }, res: Response, next: Function) => {
...
adminRouter.get('/user-details', async (req: Request & { user?: any }, res: Response) => {
  try {
...
adminRouter.get('/orders', async (req: Request, res: Response) => {
  const { gateway, clientId, limit = '20', offset = '0' } = req.query as { 
    gateway?: string, 
    clientId?: string,
    limit?: string,
    offset?: string 
  ...
  try {
    ...
    if (clientId) query = query.eq('client_id', clientId); // Assume a coluna 'client_id' da migração
...
adminRouter.post('/payments/:paymentId/cancel', async (req: Request, res: Response) => {
...
adminRouter.get('/payments/:paymentId/details', async (req: Request, res: Response) => {
    const { paymentId } = req.params;
    const { clientId } = req.query as { clientId?: string };

    if (!clientId) return res.status(400).json({ error: 'clientId query parameter is required.' });
    if (!paymentId) return res.status(400).json({ error: 'paymentId path parameter is required.' });
...
adminRouter.get('/gateway-configurations', async (req: Request, res: Response) => {
  const { clientId, gateway_provider } = req.query as { clientId?: string, gateway_provider?: string };
  ...
  try {
    ...
    if (error) throw error;
    if (!data) return res.status(404).json({ message: 'Configuration not found.'});
...
adminRouter.post('/gateway-configurations', async (req: Request, res: Response) => {
...
adminRouter.put('/gateway-configurations/:configId', async (req: Request, res: Response) => {
  const { configId } = req.params;
  const { client_id, gateway_provider, ...settings } = req.body; // client_id e provider podem não ser atualizáveis desta forma
  
  if (!configId) return res.status(400).json({ error: 'Config ID is required.'});
  ...
  try {
    const { data, error } = await supabaseAdmin
      ...
      .single();
    if (error) throw error;
    if (!data) return res.status(404).json({ error: 'Configuration not found to update.'});
    res.json({ config: data });
  } catch (err: any) {
 ...
 */
adminRouter.get('/email-domains', async (req: Request, res: Response) => {
  try {
 ...
 */
adminRouter.post('/email-domains', async (req: Request, res: Response) => {
  try {
    const {
      domain_name,
      from_name,
    ...
    const { data: newDomain, error } = await supabaseAdmin
      ...
      .insert([{
        ...
        created_by: (req as any).user?.id || null // Do middleware de autenticação
      }])
      .select()
      .single();
 ...
 */
adminRouter.put('/email-domains/:id', async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
    const {
      domain_name,
 ...
 */
adminRouter.delete('/email-domains/:id', async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
 ...
 */
adminRouter.post('/email-domains/:id/toggle', async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
 ...
 */
adminRouter.post('/email-domains/validate-api-key', async (req: Request, res: Response) => {
  try {
 ...
 */
adminRouter.get('/email-domains/:id/audit', async (req: Request, res: Response) => {
  try {
    const { id } = req.params;

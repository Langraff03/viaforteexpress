...
import { queuePaymentCreation, PaymentCreationJobData } from '../lib/queue';
import { supabaseAdmin } from '../lib/server/supabaseAdmin'; // ✅ Backend seguro // For creating an initial order record

// Define a type for the request body when creating a payment
interface CreatePaymentRequestBody {
  clientId: string; // Identifier for the client/tenant
  gatewayId: string; // Identifier for the gateway to be used
  orderId?: string; // Optional: if the order ID is known upfront
  items: Array<{
    id: string;
    name: string;
    quantity: number;
    unitPrice: number; // Price in smallest currency unit (e.g., cents)
  }>;
  customer: {
    name: string;
    email: string;
    phone?: string;
    document?: string; // e.g., CPF/CNPJ
  };
}

const router = Router();

router.post('/create-payment', async (req: Request<{}, {}, CreatePaymentRequestBody>, res: Response) => {
  try {
    ...
    const jobData: PaymentCreationJobData = {

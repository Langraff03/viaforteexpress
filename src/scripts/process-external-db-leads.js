...
let pg, mysql, mssql;
try { pg = require('pg'); } catch (e) { /* Opcional */ }
try { mysql = require('mysql2/promise'); } catch (e) { /* Opcional */ }
try { mssql = require('mssql'); } catch (e) { /* Opcional */ }
...
async function processExternalDbLeads(dbConfig, ofertaConfig, batchSize = 100) {
  ...
  try {
    ...
    switch (dbConfig.type.toLowerCase()) {
      case 'postgres':
      case 'postgresql':
        if (!pg) throw new Error('Módulo "pg" não instalado. Execute: npm install pg');
        connection = new pg.Pool(dbConfig.connection);
        break;
        
      case 'mysql':
        if (!mysql) throw new Error('Módulo "mysql2" não instalado. Execute: npm install mysql2');
        connection = await mysql.createConnection(dbConfig.connection);
        break;
        
      case 'mssql':
      case 'sqlserver':
        if (!mssql) throw new Error('Módulo "mssql" não instalado. Execute: npm install mssql');

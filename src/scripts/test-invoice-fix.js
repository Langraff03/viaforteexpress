...
// Executa: node src/scripts/test-invoice-fix.js

const { supabaseAdmin } = require('../lib/server/supabaseAdmin.ts'); // ✅ Backend seguro
...
async function testInvoiceFix() {
  ...
  try {
    ...
    testOrder.order_items.forEach((item, index) => {
      ...
      console.log(`      Total: ${item.quantity * item.unit_price}`);
    });
    ...
    // 6. Verificar total
    const calculatedTotal = invoiceData.data.itens.reduce((sum, item) => 

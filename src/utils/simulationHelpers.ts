import { addDays, addHours, addMinutes, format, setHours, setMinutes, startOfDay, parse } from 'date-fns';
...
// Retorno da função de cálculo de timestamp
export interface SubEventTimestampResult {
  date: Date | null;
  formatted: string | null;
}

type Region = 'Sudeste' | 'Sul' | 'Nordeste' | 'Centro-Oeste' | 'Norte' | 'Indefinida';

// --- Dados Fictícios e Rotas Simuladas --- 

// Mapeamento de Estado (UF) para Região (usado como fallback se rota específica não existir)
const stateToRegionMap: Record<string, Region> = {
...
const simulatedRoutes: Record<string, string[]> = {
  // Sudeste
...
export const fakeReasons: string[] = [
...
export function getRandomItem<T>(array: T[], seed: string): T {
  if (!array || array.length === 0) {
    if (typeof array[0] === 'string') return '' as unknown as T;
...
export function getSimulatedRoute(destinationState: string | null | undefined): string[] {
...
export function getBaseDateForStatus(statusKey: StepKey, order: Order): Date | null {
  ...
  const redeliveryStartDays: Partial<Record<StepKey, number>> = {
...
export function calculateSubEventTimestamp(
  subEvent: SubEvent,
  baseDate: Date | null,
  order: Order
): SubEventTimestampResult {
  const fallbackResult: SubEventTimestampResult = { date: null, formatted: null };
  if (!baseDate || isNaN(baseDate.getTime())) return fallbackResult;
...
export function getEstimatedDeliveryWindow(baseDate: Date | null): string | null {

...
import { AssetGateway, AssetGatewayConfig } from '../../lib/gateways/AssetGateway';

interface AssetWebhookRequest {
  type: string;
  data: {
    id: number | string;
    amount: number;
    fee?: { netAmount?: number };
    status: 'pending' | 'paid' | 'confirmed' | 'received' | 'cancelled' | 'refunded' | 'waiting_payment' | 'refused';
    customer: {
      name: string;
      email: string;
      address?: {
    ...
    // Adicionando campos necessários para o formato do sistema antigo
    secureId?: string;
    secureUrl?: string;
    paidAt?: string;
    items?: Array<{
...
export const assetWebhookHandler = async (req: Request, res: Response) => {
  ...
  try {
    const payload = req.body as AssetWebhookRequest;
    // Extrair client_id da query string ou usar um valor padrão
    let clientId = (req.query.client_id as string) || 'default';
    ...
    const hasShippingAddress = !!(
      payload.data.shipping?.address?.street ||
    ...
    await queuePaymentWebhook({
      ...
      payload: {
        ...
        customer: {
          ...
          phone: (d.customer as any)?.phone,      // ✅ ADICIONAR TELEFONE
          document: (d.customer as any)?.document  // ✅ ADICIONAR CPF
        }
      }
    });

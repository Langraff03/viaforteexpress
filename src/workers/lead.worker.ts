...
const internalLeadWorker = new Worker<InternalLeadJobData>(
  'internal-lead',
  async (job: Job<InternalLeadJobData>) => {
    ...
    try {
      ...
      async function fetchLeadsBatch(page: number, pageSize: number) {
      ...
      const pendingBatches = new Set<Promise<{ success: number; fail: number }>>();
...
const externalLeadWorker = new Worker<ExternalLeadJobData>(
  'external-lead',
  async (job: Job<ExternalLeadJobData>) => {
    ...
    try {
      ...
      // Processar lotes com concorrência limitada
      let activeBatches = 0;
      const batchPromises: Promise<{ success: number; fail: number }>[] = [];
      const pendingBatches = new Set<Promise<{ success: number; fail: number }>>();
...
const leadEmailWorker = new Worker<LeadEmailJobData>(
  'lead-email',
  async (job: Job<LeadEmailJobData>) => {
...
export async function processCSVFile(filePath: string, clientId: string, ofertaConfig: any) {
  try {
    // Ler o arquivo
    const fileContent = fs.readFileSync(filePath, 'utf8');
    
    // Parsear o CSV de forma assíncrona
    const parseAsync = (content: string): Promise<any[]> => {
...
export async function processJSONFile(filePath: string, clientId: string, ofertaConfig: any) {
  try {
    // Ler o arquivo
    const fileContent = fs.readFileSync(filePath, 'utf8');
